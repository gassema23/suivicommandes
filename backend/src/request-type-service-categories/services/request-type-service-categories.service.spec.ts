...
describe('RequestTypeServiceCategoriesService', () => {
  ...
  beforeEach(async () => {
    createQueryBuilderMock = {
      ...
      getManyAndCount: jest.fn().mockResolvedValue([[mockRequestTypeServiceCategory], 1]),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RequestTypeServiceCategoriesService,
        {
          ...
          useValue: {
            findAndCount: jest.fn().mockResolvedValue([[mockRequestTypeServiceCategory], 1]),
        ...
        },
        {
          ...
          useValue: {
            findOne: jest.fn().mockResolvedValue({ id: 'uuid-request-type', requestTypeName: 'Type A' }),
          },
        },
        {
          provide: getRepositoryToken(ServiceCategory),
          useValue: {
            findOne: jest.fn().mockResolvedValue({ id: 'uuid-service-category', serviceCategoryName: 'Catégorie A' }),
          },
        },
      ],
    }).compile();
    ...
    repo = module.get<Repository<RequestTypeServiceCategory>>(getRepositoryToken(RequestTypeServiceCategory));
  });
  ...
  it('should throw if request type service category already exists on create', async () => {
    (repo.findOne as jest.Mock).mockResolvedValueOnce(mockRequestTypeServiceCategory);
    await expect(
      service.create(
        {
          serviceCategoryId: 'uuid-service-category',
  ...
  it('should find one request type service category', async () => {
    createQueryBuilderMock.getOne.mockResolvedValueOnce(mockRequestTypeServiceCategory);
  ...
  it('should update a request type service category', async () => {
    createQueryBuilderMock.getOne.mockResolvedValueOnce(mockRequestTypeServiceCategory); // findOne(id)
  ...
  it('should remove a request type service category', async () => {
    ...
    expect(repo.save).toHaveBeenCalled();
    expect(repo.softDelete).toHaveBeenCalledWith('uuid-request-type-service-category');
  });

  it('should throw if request type service category not found on remove', async () => {
    createQueryBuilderMock.getOne.mockResolvedValueOnce(undefined);
    await expect(service.remove('not-exist', 'user-id')).rejects.toThrow(BadRequestException);
  });
});