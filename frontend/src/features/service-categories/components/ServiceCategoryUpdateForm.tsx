...
export default function ServiceCategoryUpdateForm({
  serviceCategory,
  ...
  const form = useForm<ServiceCategoryFormData>({
    ...
    defaultValues: {
      ...
      sectorId: serviceCategory.service.sector.id ?? "",
      serviceId: serviceCategory.service.id ?? "",
      isMultiLink: serviceCategory.isMultiLink ?? false,
      isMultiProvider: serviceCategory.isMultiProvider ?? false,
      isRequiredExpertise: serviceCategory.isRequiredExpertise ?? false,
    },
  });
  ...
  const updateServiceCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(SUCCESS_MESSAGES.update("Catégorie de service"));
  ...
  const onSubmit = (data: ServiceCategoryFormData) => {
    updateServiceCategoryMutation.mutate(data);
  };
  ...
  return (
    ...
    >
      ...
      <FormActions
        isLoading={updateServiceCategoryMutation.isPending}
        onCancel={() =>
          navigate({ to: "/pilotages/service-categories", search: { page: 1 } })
        }
      />
