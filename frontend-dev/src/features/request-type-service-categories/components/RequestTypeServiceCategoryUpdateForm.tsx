...
import { fetchRequestTypeList } from "@/features/request-types/services/fetch-request-type-list.service";
...
import { requestTypeServiceCategorySchema, type RequestTypeServiceCategoryFormData } from "../schemas/request-type-service-category.schema";
...
export default function RequestTypeServiceCategoryUpdateForm({
  ...
  const form = useForm<RequestTypeServiceCategoryFormData>({
    ...
    defaultValues: {
      ...
      minimumRequiredDelay: requestTypeServiceCategory.minimumRequiredDelay || 0,
      serviceActivationDelay: requestTypeServiceCategory.serviceActivationDelay || 0,
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: RequestTypeServiceCategoryFormData) => updateRequestTypeServiceCategory(requestTypeServiceCategory.id, data),
    onSuccess: () => {
      setBackendError(null);
      toast.success(SUCCESS_MESSAGES.update("Catégories de services par type de demande"));
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.REQUEST_TYPE_SERVICE_CATEGORIES });
      navigate({ to: "/pilotages/request-type-service-categories", search: { page: 1 } });
    },
